...
app.get('/:uniqueString', async (req, res) => {
    ...
    const link = await Link.findOne({ generatedURL: 'https://adlogx.wixsite.com/' + uniqueString });
...
async function generateUniqueLink() {
    ...
    while (!isUnique) {
        generatedURL = 'https://adlogx.wixsite.com/' + generateRandomCharacters(8, characters);
